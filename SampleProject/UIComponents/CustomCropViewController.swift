...
class CustomCropViewController: CropViewController {
    ...
    override func viewDidLoad() {
        ...
        // Access cropToolbar using reflection
        if let toolbar = self.value(forKey: "cropToolbar") as? CropToolbarProtocol {
            toolbar.delegate?.didSelectRatio(toolbar, ratio: 1)
        }
        ...
        // Access stackView using reflection
        if let stackView = self.value(forKey: "stackView") as? UIStackView {
        ...
        // Access cropStackView using reflection
        if let cropStackView = self.value(forKey: "cropStackView") as? UIStackView {
...
extension CustomCropViewController: UITextFieldDelegate {
    ...
    @objc func textDidChange() {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            // Access cropToolbar using reflection
            if let toolbar = self.value(forKey: "cropToolbar") as? CropToolbarProtocol {
                toolbar.delegate?.didSelectRatio(toolbar, ratio: width / height)
            }
        }
    }
